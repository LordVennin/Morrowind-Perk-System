...
- Engine objects (active spells, their effects, game objects) are USERDATA, not Lua tables. A `type(x) == "table"` guard silently drops them while field access would have worked fine; guard with `type(x) == "table" or type(x) == "userdata"`, or test for the field you need instead of the container type.
- Never use a GameObject as a table key: userdata hashes by wrapper identity, so the same actor collects one entry per wrapper and set semantics silently break. Key by `actor.id`.
- The `SpellCasting` local interface does not exist in OpenMW 0.51.0 or earlier (master-only at time of writing). To react to a spell landing on an actor on released builds, have the caster's script report the cast (skill-used handler gives the moment and `getSelectedSpell` the spell id) and let the target scan its own `Actor.activeSpells` for a fresh instance of that spell id during a short window - projectiles take flight time, so the window must outlive the cast by a few seconds.
- Check the docs version against the player's actual build before designing around an interface: "latest" documents master, and the log header prints the running version and revision.
...
- A perk that pays the player back in health, fatigue or magicka must not do it by writing `stat.current`. Topping up a pool that is already full is completely invisible, and right after casting a cheap spell the pool usually is full, so the perk reads as broken even when every step of the pipeline ran. Apply a Fortify active spell for a duration instead: it shows on the bar whatever the pool was at.
- `string.gsub` returns the result *and* a replacement count. A normalising helper written as `return value:lower():gsub("%s+", "")` therefore returns two values, which silently becomes an extra argument at any call site that forwards it. Assign to a local and return that local.
- Engine lists (an active spell's `effects`, for instance) are userdata, and not all of them answer `pairs()`. Flatten to a plain array with `ipairs` first, falling back to `pairs`, before classifying.
- An interface override (`interfaceName = "Combat"`) must answer for EVERY field of the interface it replaces, constants included, not just the functions it wraps. Other mods read `I.Combat.ATTACK_SOURCE_TYPES` and the like straight off the interface, and a `Hit` handler that throws on a missing constant aborts the engine's handler chain before its default damage handler runs, so no melee hit lands while this mod is loaded. Give the override table a metatable whose `__index` falls through to the base interface captured in `onInterfaceOverride`, and add only what you deliberately change on top.
- Per-cast and per-frame tracing must sit behind a debug flag that defaults to off and can be toggled in game (see `spsdestructiondebug`). Shipping it on drowns the log and hides the one line that matters.
...
- Hidden perks: a perk entry may carry `hiddenUnless = { race, majorSkills, classSkills, classSkillsAny, contentFiles, label }` (see `register.lua`). The check becomes the node's visibility test in the menu and its first purchase requirement, and an owned perk stays visible. Keep every engine read inside it pcall-wrapped so `tools/uitest.lua`'s mocks and a missing API hide the perk instead of breaking the page.
- Short timed effects on the player or on a named actor go through the generic `SkillPerkSystem_BasePack_SkillBase_SelfRider` / `_TargetRider` global events (`{ player, target?, kind, name, effects = { { effect, fallback, magnitude, seconds, skill?, attribute? } } }`). One instance per `kind` is held at a time, records are cached and saved; do not mint another per-tree rider pipeline for something these cover.
- The player script samples an idle subsystem's `shouldUpdate` only every 0.1s with that frame's `dt`, so a `dt` accumulator in `shouldUpdate` reaches a 0.5s threshold several seconds late. Gate polls that must actually run twice a second on `core.getRealTime()` deadlines instead (see Berserker's Blood in `blunt_weapon.lua`).
- Custom magic effects ARE possible on OpenMW 0.51+, but only from a `LOAD:` script through `openmw.content` (`content.magicEffects.records[id] = { template = <existing effect record>, name, baseCost, icon, description }`), never from `world.createRecord` at runtime. A custom effect inherits its template's look (school, icon, sounds) and none of its engine behaviour, so what it does must be scripted: watch `Actor.activeEffects(actor):getEffect(id)` or the actor's `activeSpells` and act. Records injected at load are not saved, which is fine because the load script runs on every launch; runtime code must still check the record exists before granting it (`core.magic.spells.records[id]`). See `basepack_load.lua` and `TRCustomSpells/` for a worked example.
