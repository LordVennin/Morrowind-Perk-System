...
local function maxSkillPages()
    return math.max(1, math.ceil(#skillIDs / SKILLS_PER_PAGE))
end

local function clampSkillPage()
    local maxPage = maxSkillPages()
    if currentSkillPage < 1 then
        currentSkillPage = 1
    elseif currentSkillPage > maxPage then
        currentSkillPage = maxPage
    end
end

local function changeSkillPage(delta)
    currentSkillPage = currentSkillPage + delta
    clampSkillPage()

    local startIndex = (currentSkillPage - 1) * SKILLS_PER_PAGE + 1
    local endIndex = math.min(startIndex + SKILLS_PER_PAGE - 1, #skillIDs)
    if selectedSkillIndex < startIndex or selectedSkillIndex > endIndex then
        selectedSkillIndex = startIndex
        selectedPerkIndex = 1
        updateFilteredPerks()
    end
...
local function buildSkillTabs()
    ...
    local startIndex = (currentSkillPage - 1) * SKILLS_PER_PAGE + 1
    local endIndex = math.min(startIndex + SKILLS_PER_PAGE - 1, count)
    local pageTabs = buildSkillTabRow(startIndex, endIndex)
            ...
            },
            {
                ...
                content = ui.content {
                    createButton("<", function()
                        changeSkillPage(-1)
                    end, currentSkillPage > 1, util.vector2(28, 24)),
                    {
                        type = ui.TYPE.Widget,
                        props = { size = util.vector2(8, 1) },
                    },
                    {
                        ...
                        template = interfaces.MWUI.templates.textNormal,
                        props = {
                            text = string.format("Page %d / %d", currentSkillPage, maxSkillPages()),
                            autoSize = true,
                        },
                    },
                    {
                    ...
                    createButton(">", function()
                        changeSkillPage(1)
                    end, currentSkillPage < maxSkillPages(), util.vector2(28, 24)),
                },
            },
        }
    }
...
local function showMenu()
    ...
    currentSkillPage = math.floor((selectedSkillIndex - 1) / SKILLS_PER_PAGE) + 1
    clampSkillPage()
    selectedPerkIndex = 1
    updateFilteredPerks()
