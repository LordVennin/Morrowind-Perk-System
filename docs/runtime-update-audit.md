...
| Combined player engine chain | generated `__combinedEngineHandlers[handlerName]` | Builds a chain for every subsystem handler with the same engine event; for `onUpdate` and `onFrame`, invokes each subsystem contribution sequentially. | Required by current consolidation architecture; amplifies all polling costs. | This is the central reason every subsystem's update/frame work is always on the player path. Per-subsystem gating would reduce cost without splitting scripts. |
