...
`basepack_player.lua` now only requires one module per perk tree from `runtime/player/` and combines their handlers into a single player script engine-handler chain. Each subsystem listed below contributes to the same player `onUpdate`/`onFrame` path. Rows naming a file refer to that module; the rest were extracted from the former consolidated initializers of the same name.
...
| Athletics (`runtime/player/athletics.lua`) | `onUpdate` / `shouldUpdate` | Builds and decays Stride stacks from sampled movement, applies Athletics/Endurance/Speed/max-fatigue modifiers, and runs the Second Wind cooldown. | Timer-based; no per-frame path by design. | `0.5`-second poll. Stride build (2s), decay (3s) and the Second Wind cooldown (30-60s) all have second-level granularity, so a nonzero applied bonus is deliberately **not** treated as a reason to keep updating. Movement is sampled from player position once per poll rather than through a watcher. |
| Acrobatics (`runtime/player/acrobatics.lua`) | `onUpdate` / `shouldUpdate` | Tracks airborne/grounded state, applies Acrobatics/Agility/Speed modifiers, restores fatigue on landing, and runs the Rooftop Runner cooldown. | Timer-based; no per-frame path. | `0.25`-second poll. Faster than the other trees because a jump only lasts about a second; this is the faster cadence the budget allows for sampled state. Swimming is excluded from "airborne" explicitly. |
| Sneak (`runtime/player/sneak.lua`) | `onUpdate` / `shouldUpdate` | Applies Sneak/Security/Agility/Speed modifiers from sneak state, stillness, carried light, and encumbrance. | Timer-based; no per-frame path. | `0.5`-second poll. All four conditions are cheap reads (`self.controls.sneak`, position delta, one equipment slot, encumbrance ratio). |
